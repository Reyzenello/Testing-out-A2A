...
def agent_card():
    ...
    return jsonify({
        ...
        "url": "http://localhost:5000",  # Where this agent is hosted (used by clients)
        "version": "1.0",  # Version info for the agent
        "capabilities": {
            "streaming": False,  # Indicates that this agent does not support streaming
            "pushNotifications": False  # Indicates that the agent does not support push notifications
        }
    })
...
def handle_task():
    try:
        ...
        # A2A represents messages as a list of "parts", where the first part usually contains the user's text.
        user_message = task["message"]["parts"][0]["text"] # Assuming the user message is in the first part

        # --- Agent Logic: Respond with the current time ---
        ...
        app.logger.error(f"Error processing task: {e}, Task received: {request.data}")
...
if __name__ == "__main__":
    app.run(port=5000, debug=True) # debug=True is helpful for development
